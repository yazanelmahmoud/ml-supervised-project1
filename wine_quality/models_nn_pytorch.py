"""
Neural Network — PyTorch MLP for Wine Quality (Multiclass Classification).
SGD only (no momentum). Same 4-step pipeline as sklearn: width, depth, LR sweep, final model.
Uses class_weight balanced. Same plots as sklearn NN. Macro-F1 for multiclass.
"""
...
def _encode_labels(y):
    """Map quality labels (e.g. 3,4,5,6,7,8,9) to 0..n_classes-1."""
    uniq = np.unique(y)
    return np.array([np.searchsorted(uniq, v) for v in y], dtype=np.int64), uniq


def _decode_labels(y_encoded, unique_classes):
    """Map 0..n_classes-1 back to original labels."""
    return unique_classes[y_encoded]


def _get_max_epochs(n_samples):
    return min(500, max(100, n_samples // BATCH_SIZE * 100))
...
def _fit_one_fold(X_tr, y_tr_enc, X_val, y_val_enc, arch, lr, max_epochs, class_weight_tensor, n_classes, device):
    """Train on one fold; return (train_f1_macro, val_f1_macro), loss_curve."""
    n_features = X_tr.shape[1]
    model = MLP(n_features, arch, output_size=n_classes).to(device)
    opt = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.0, weight_decay=NN_L2)
    criterion = nn.CrossEntropyLoss(weight=class_weight_tensor)

    ds = TensorDataset(
        torch.FloatTensor(X_tr).to(device),
        torch.LongTensor(y_tr_enc).to(device)
    )
    ...
    for epoch in range(max_epochs):
        ...
        model.eval()
        with torch.no_grad():
            pred_val = model(torch.FloatTensor(X_val).to(device)).argmax(dim=1).cpu().numpy()
            val_f1 = f1_score(y_val_enc, pred_val, average='macro', zero_division=0)
...
def _fit_one_fold_width(w, max_epochs, train_idx, val_idx, X_train, y_train_enc, class_weight, n_classes):
    ...
    tr_f1, v_f1, _ = _fit_one_fold(
        ...
        (w,), 0.01, max_epochs, class_weight_tensor, n_classes, device
    )
    return (tr_f1, v_f1)


def _fit_one_fold_arch(arch, max_epochs, train_idx, val_idx, X_train, y_train_enc, class_weight, n_classes):
    ...
    tr_f1, v_f1, _ = _fit_one_fold(
        X_tr, y_tr, X_val, y_val,
        arch, 0.01, max_epochs, class_weight_tensor, n_classes, device
    )
    return (tr_f1, v_f1)


def _fit_one_fold_lr(arch, lr, max_epochs, train_idx, val_idx, X_train, y_train_enc, class_weight, n_classes):
    ...
    tr_f1, v_f1, _ = _fit_one_fold(
        X_tr, y_tr, X_val, y_val,
        arch, lr, max_epochs, class_weight_tensor, n_classes, device
    )
    return (tr_f1, v_f1)


def _fit_one_fold_lc(arch, lr, max_epochs, train_idx, val_idx, X_train, y_train_enc, class_weight, n_classes):
...
def run_nn_step1(X_train, y_train, X_test, y_test, cv=CV_SPLITS):
    ...
    class_weight = compute_class_weight("balanced", classes=np.arange(n_classes), y=y_train_enc)
    class_weight_tensor = torch.FloatTensor(class_weight).to(device)

    with open(NN_PYTORCH_RESULTS_PATH, "w", encoding="utf-8") as f:
        f.write("NN (PyTorch) pipeline results — Wine Quality [class_weight=balanced]\n")
        f.write("=" * 60 + "\n\n")

    cv_splitter = StratifiedKFold(n_splits=cv, shuffle=True, random_state=RANDOM_SEED)
    n_samples = X_train.shape[0]
    max_epochs = _get_max_epochs(n_samples)

    # Parallelize CV folds across widths
    folds_list = list(cv_splitter.split(X_train, y_train_enc))
    tasks = [
    ...
    results = Parallel(n_jobs=N_JOBS)(
        delayed(_fit_one_fold_width)(w, max_epochs, train_idx, val_idx, X_train, y_train_enc, class_weight, n_classes)
    ...
    results = {
        "widths": WIDTH_VALUES,
        "train_f1_macro": train_f1_list,
        "cv_f1_macro": cv_f1_list,
...
def run_nn_step2(X_train, y_train, X_test, y_test, nn_step1, cv=CV_SPLITS):
    ...
    results = Parallel(n_jobs=N_JOBS)(
        delayed(_fit_one_fold_arch)(arch, max_epochs, train_idx, val_idx, X_train, y_train_enc, class_weight, n_classes)
...
def run_nn_step3(X_train, y_train, X_test, y_test, nn_step2, cv=CV_SPLITS):
    ...
    results = Parallel(n_jobs=N_JOBS)(
        delayed(_fit_one_fold_lr)(best_arch, lr, max_epochs, train_idx, val_idx, X_train, y_train_enc, class_weight, n_classes)
    ...
    _, _, loss_curve = _fit_one_fold(
        X_train, y_train_enc, X_train, y_train_enc,
        best_arch, best_lr, max_epochs, class_weight_tensor, n_classes, device
    )
...
def run_nn_step4(X_train, y_train, X_test, y_test, nn_step3, cv=CV_SPLITS):
    ...
    # Learning curve - parallelize CV folds
    ...
    for frac in LEARNING_CURVE_TRAIN_SIZES:
        ...
        results = Parallel(n_jobs=N_JOBS)(
            delayed(_fit_one_fold_lc)(best_arch, best_lr, max_epochs, train_idx, val_idx, X_sub, y_sub, class_weight, n_classes)
    ...
    model = MLP(n_features, best_arch, output_size=n_classes).to(device)
    ...
    with torch.no_grad():
        ...
        y_pred = _decode_labels(pred_enc, unique_classes)
    ...
    _write_step4_results(
        cm, metrics, fit_time, predict_time,
        train_sizes_abs, train_scores, val_scores,
...
def _write_step4_results(cm, metrics, fit_time, predict_time,
    ...
    unique_classes = sorted([int(k.split('_')[1]) for k in f1_per_class.keys()])
    ...
    for class_label in unique_classes:
        ...
        f1_val = f1_per_class.get(class_key, 0.0)
        acc_val = accuracy_per_class.get(class_key, 0.0)
        lines.append(f"  Quality {class_label}: F1 = {f1_val:.4f}, Accuracy = {acc_val:.4f}")
    lines.extend([
        "",
